...
import { getMovieDetail, getMovieReviews } from '../../api/getMovie';
...
getMovieDetail(movieId).then((response) => {
  ...
  movieVoteCount.innerHTML = `${response.vote_count} votes`;
  ...
  movieReleaseDate.innerHTML = response.release_date.slice(0, 4);
  movieRuntime.innerHTML = `${Math.floor(response.runtime / 60)}h ${
    response.runtime % 60
  }min`;
